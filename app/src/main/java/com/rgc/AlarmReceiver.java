                            ...
                            if(!notifText.isEmpty()){
                                ...
                                if(!powK.isNull(8))
                                    if(!powK.getString(8).isEmpty()&&!powK.getString(8).equals("Not selected")) {
                                        Uri selectedSound = Uri.parse(powK.getString(8));
                                        builder.setSound(selectedSound);
                                    }
                                NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                    NotificationChannel channel = new NotificationChannel("M_CH_ID","RGC Channel", NotificationManager.IMPORTANCE_DEFAULT);
